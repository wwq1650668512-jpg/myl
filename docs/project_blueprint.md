...
`A_i(t+1) = A_i(t) * exp(base_growth_i + drug_effect_i + ecology_i)`

`D_j(t+1) = D_j(t) - sum_i metabolism_rate_ij * A_i(t)`

`P_k(t+1) = P_k(t) + sum_(i,j) product_flux_ijk - clearance_k`

`H(t+1) = f(taxa, metabolites, diversity, GMWI2_like_score)`
...
`development_score = efficacy_proxy - dysbiosis_penalty - uncertainty_penalty`
