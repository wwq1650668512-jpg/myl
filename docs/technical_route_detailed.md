...
3. 用 Step 2 的 `predicted_metabolized_probability` 和 `predicted_parent_depletion_fraction` 近似药物代谢消耗；
4. 用一个简化的 `aggregate metabolite pool` 累积代谢负荷；
5. 每个时间点计算健康指数和开发评分。
...
microbe_abundance(t+1)
    = normalize(
        microbe_abundance(t)
        * exp(effect_scale * step1_effect * drug_exposure + ecology_pull)
      )
...
当前健康指数是 `GMWI2-like heuristic`，并不是官方 GMWI2 终值。它由 4 个分量加权构成：
...
health_index = 100 * (0.40 * diversity + 0.35 * balance + 0.25 * stability)
```
...
这里再次强调，`GMWI2-like` 的意思是“受 GMWI2 思路启发”，不是官方 GMWI2 的严格复现。
